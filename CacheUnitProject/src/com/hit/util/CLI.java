...
public class CLI implements Runnable {
	...
	public void run() {
		...
		while(flag) {
			command[0] = input.nextLine().toLowerCase(); // Only lower characters for command
			while(!command[0].equals(START) && !command[0].equals(SHUTDOWN)) {
				write("Not a valid command");
				command[0] = input.nextLine().toLowerCase();
			}
			...
			case START:// The user need put here configuration for server 
				if(start) { // Only single start order
					...
					command[1] = input.nextLine().toUpperCase(); // Only upper characters for command
					while(!command[1].equals(LRU) && !command[1].equals(NRU) && !command[1].equals(RANDOM)) {
						write("Not a valid command");
						command[1] = input.nextLine().toUpperCase();
					}
			...
			case SHUTDOWN:
				...
				flag = false;
				if(!start) {
					listener.firePropertyChange(CLI.SHUTDOWN, null, command);	//Send to Server
				}
				input.close();
				output.close();
			}
